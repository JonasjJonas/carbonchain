...
/**
 * @title CCTArr
 * @notice CarbonChain Carbon Token — Restauração e Reflorestamento
 *
 * Símbolo:      CCT-ARR
 * Metodologia:  VM0047 (Afforestation, Reforestation and Revegetation)
 ...
 */
contract CCTArr is ERC20, AccessControl {
    ...
    constructor(address admin) ERC20("CarbonChain ARR Credit", "CCT-ARR") {
    ...
    function mintLote(
        ...
        require(quantidade > 0, "CCT-ARR: quantidade zero");

        bytes32 lote_id = keccak256(abi.encodePacked(cpa_id, periodo));
        require(lotes[lote_id].emitido_em == 0, "CCT-ARR: lote ja mintado");
    ...
    function retire(uint256 quantidade, string calldata motivo) external {
        require(quantidade > 0, "CCT-ARR: quantidade zero");
        require(balanceOf(msg.sender) >= quantidade * 1e18, "CCT-ARR: saldo insuficiente");
    ...
    {
        require(lotes[lote_id].emitido_em > 0, "CCT-ARR: lote inexistente");
