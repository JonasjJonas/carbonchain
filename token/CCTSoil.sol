...
/**
 * @title CCTSoil
 * @notice CarbonChain Carbon Token — Solo Agrícola
 *
 * Símbolo:      CCT-SOIL
 * Metodologia:  VM0042 v2.2 (Improved Agricultural Land Management)
 ...
 */
contract CCTSoil is ERC20, AccessControl {
    ...
    struct Lote {
        ...
        string  periodo;
        bytes32 hash_mrv;
        string  vcu_id;       // preenchido após emissão na Verra
        uint256 emitido_em;
    }
    ...
    constructor(address admin) ERC20("CarbonChain Soil Credit", "CCT-SOIL") {
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
        _grantRole(MINTER_ROLE, admin);
    }

    /**
     * @notice Minta créditos de solo para um lote verificado.
     * Chamado exclusivamente pelo CCTFactory após Voucher do Cartesi.
     */
    function mintLote(
        ...
        require(quantidade > 0, "CCT-SOIL: quantidade zero");

        bytes32 lote_id = keccak256(abi.encodePacked(cpa_id, periodo));
        require(lotes[lote_id].emitido_em == 0, "CCT-SOIL: lote ja mintado");
    ...
    /**
     * @notice Aposenta créditos — comprova neutralização de emissões.
     * Queima os tokens permanentemente.
     */
    function retire(uint256 quantidade, string calldata motivo) external {
        require(quantidade > 0, "CCT-SOIL: quantidade zero");
        require(balanceOf(msg.sender) >= quantidade * 1e18, "CCT-SOIL: saldo insuficiente");
    ...
    {
        require(lotes[lote_id].emitido_em > 0, "CCT-SOIL: lote inexistente");
